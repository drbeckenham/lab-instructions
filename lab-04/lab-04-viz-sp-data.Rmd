...
What are the dimensions of the Denny's dataset? What does each row in the dataset represent? What are the variables? There are 1643 rows and 6 columns.
...
**Your answer:** The Denny's dataset has 1643 rows and 6 columns. Each row represents a specific Denny's. The variables are address, city, state, zip, longitude, and latitude.
...
**Your answer:** The La Quinta dataset has 909 rows and 6 columns. Each row represents a specific La Quinta Inn. The variables are ddress, city, state, zip, longitude, and latitude.
...
- Locations include Canada, Mexico, Honduras, Colombia, Chile, turkey, China, Istanbul, Turkey, United Arab Emirates, and future expansion planned in India, Nepal, and Bangladesh.
- 
- 

**Denny's locations outside the US:**
- I had to search outside of the given link. As of 2024, there were more than 165 international locations. These locations included:
Canada, Mexico, Puerto Rico, Phillippines, New Zealand, Honduras, United Arab Emirates, Costa Rica, El Salvador, Guatemala, Guam, INdonesia, Curacao, United Kingdom.
- 
- 
...
**Your ideas:**
- I would search and exclude USA. The data set for LaQuinta is not ordered, so I would do that first. Dann's seems to be in order by state. I don't see other countries listed,.
- 
- 
...
**How many Denny's locations are outside the US?** It shows 0, although we know that is different from our previous online search.

**If there are any, which state/province codes appear?** It doesn't show anything.
...
- State with fewest locations: Vermont

**Is this surprising? Why or why not?**

There doesn't seem to be any correlation one with the other So, based on the initial premise of this exercise I was surprised.
...
Rhode Island. New Jersey, and Connecticut have the most Dennys per 1000 square miles. For LaQuinta, the states with the most per 1,000 miles is New Jersey, Rhode Island and Delaware.
...
- Density value: 6.1 per 1000 miles

**La Quinta:**

- State with highest density: New Jersey_
- Density value: 1.6 per 1000 miles
...
dn_lq %>%
  filter(state == "NC") %>%
  ggplot(mapping = aes(x = longitude, y = latitude, color = state)) +
  geom_point(alpha = 0.5) +
  labs(
    title = "Denny's vs La Quinta Locations in North Carolina",
    x = "Number of Dennys'",
    y = "Number of La Quintas"
  )
```
...
It does seem to be slightly true in some circumstances. They are grouped together in clusters.
...
dn_lq %>%
  ...
  labs(
    title = "Denny's vs La Quinta Locations in Texas",
    x = "Number of Dennys'",
    y = "Number of La Quintas"
  )
```
...
It absolutely does! They are all clustered together, more so than in NC. Very interesting visual.


**Compare North Carolina and Texas: In which state does the joke seem more accurate?**


Obviously, the joke works a lot better in Texas, just by examining the clustering.
