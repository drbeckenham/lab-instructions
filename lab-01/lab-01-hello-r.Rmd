...
There are 13 rows with the following variables found in the frequency table below.
...
- Compared to the dino dataset (r = -0.06447185), the star dataset's correlation is: larger
...
- Compared to the dino dataset, the circle dataset's correlation is:smaller

- What do you notice about the correlation coefficients for dino, star, and circle?The circle is smaller and shows three outliers. The other datasets do not shw outliers, and are move complex.
...
```{r all-viz, eval=FALSE, fig.width=5}
...
```{r fig-resize-local, fig.margin = TRUE, echo = FALSE, fig.width=1}
...
```{r theme-highlight, fig.margin = TRUE, echo = FALSE, fig.width=7}
